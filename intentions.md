...
**Motivation (measured 2026-09-22, `teachbot-timeseries`):** 7 of 51 assistant turns (13.7%) on the first real class day were the timeout apology; 4 of 15 sessions hit it. One student asked the same question three times, got the apology three times, and left. Probing the deployed model against the real 10k-token system prompt showed why: `deepseek/deepseek-v4-flash-0731` is a **reasoning** model emitting 80-480 `reasoning` deltas (first at ~3s) before its first `content` delta (3-10s, long tail past 70s), while `stream_response` yielded only `delta.content` — so the watchdog could not tell "thinking hard" from "hung" and killed healthy requests.
**Reasoning counts as liveness.** `src/llm_client.py::stream_events` yields `(REASONING|CONTENT, text)` tuples (`stream_response` is now the content-only view of it, kept for the test harnesses). Two deadlines replace the single one:
- `FIRST_TOKEN_TIMEOUT_S` (15s) — maximum gap between deltas of *any* kind, i.e. "is the connection alive".
- `FIRST_CONTENT_TIMEOUT_S` (45s) — absolute ceiling from request start to the first *content* delta, so an endlessly-ruminating model still gives up.
Once content starts flowing no deadline applies — aborting there would discard a half-written answer the student can already read. Measured effect on the real model: the longest silence the watchdog sees drops from ~15s (time to first content) to ~3.5s (time to first anything). The question that failed three times for one student now answers in 8.7s.
**"Thinking…" step.** Reasoning deltas open a `cl.Step` so the student sees activity instead of an empty bubble (verified over the live websocket: step appears at 5.8s, answer at 14.8s). Its text is shown only when `show_reasoning=True`; **learning-goals mode passes False**, because a tutor's chain-of-thought spells out the answer and the mastery verdict the Socratic dialogue is meant to elicit — there the step shows a neutral "Thought for Ns." instead.
...
**Key files:** `app.py` (`_stream_once`, `_stream_assistant`, `_aclose`, `_log_stream_failure`, all three call sites), `src/llm_client.py` (`stream_events`, `REASONING`/`CONTENT`), `src/chat_logger.py` (`model` on `error` rows), `scripts/render_chats.py` (`KNOWN_ROLES` + `.msg.error` styling), `tests/stream_resilience.py`
**No-Goals:** Retrying on a *different* model, unbounded retries, resuming a partially-streamed answer, showing chain-of-thought in learning-goals mode.
...
**Lifecycle:** DONE
**Description:** Per-course educator-defined list of LLM models students can choose from during a session. Configured via `student_model_choices` (list of `{id, label, reasoning?}` entries — `reasoning` is IID-LLM-THINKING) in a course `_meta.yaml`, the root `content/_meta.yaml`, or the deploy config. **Single-course deploys** (content dir with no subfolders, e.g. `teachbot-timeseries`) read the list from the deploy config, since they have no `_meta.yaml`. The session's LLM config is a per-session *copy*, so one student's choice cannot leak into another's session. When set, Chainlit's Chat Settings gear exposes a model dropdown; changes apply from the next message. The active model is stored in every assistant log entry (JSONL `model` field + Google Sheet `model` column) and displayed as a chip below assistant bubbles in the HTML chat viewer. Inactive when `student_model_choices` is absent or empty.
**Key files:** `src/course_loader.py` (`CourseConfig` field + `parse_model_choices`), `src/chat_logger.py` (`model` column), `app.py` (`on_chat_start` ChatSettings + `on_settings_update`), `scripts/render_chats.py` (model chip), `tests/model_choices.py`

### IID-LLM-THINKING
**Lifecycle:** DONE
**Description:** Chain-of-thought is a per-config, per-model-choice switch rather than whatever the provider happens to default to. `llm.reasoning` in any config (or `reasoning` on a `student_model_choices` entry) accepts `false` (no chain-of-thought), `true`, or a mapping passed through verbatim (`{effort: low}`, `{max_tokens: 512}`); omitting it sends nothing and preserves the provider default. Travels in `extra_body` because `reasoning` is an OpenRouter extension, not an OpenAI-SDK parameter, and is applied to **both** the streamed call (`stream_events`) and the diagnose call (`complete_json`).
**Why:** a reasoning model is a poor fit for grounded Q&A over supplied lecture notes — it pays a large latency cost for chain-of-thought the student never sees. `deepseek/deepseek-v4-flash-0731` on the timeseries prompt, measured 2026-09-22: reasoning deltas 211/71/263 → **0/0/0** and time-to-first-content 11.3s/26.1s/13.4s → **1.2s/3.3s/3.0s**, with answer lengths comparable. That latency is what drove the 13.7% failure rate in IID-STREAM-RESILIENCE.
**Student-facing:** a `student_model_choices` entry may repeat the same model id under two labels differing only in `reasoning` — this is why choices are keyed by **label**, and why duplicate labels are a startup error. `teachbot-timeseries` offers "Fast (no thinking)" (the default) and "Thorough (thinks first, slower)". Selecting a choice sets model *and* reasoning together; a choice that omits `reasoning` clears it, so switching back and forth is symmetric rather than latching.
**Verify before configuring:** support is model- *and provider*-specific, and a provider that declares the parameter may still ignore it. `python scripts/check_reasoning_support.py <model> --live` reports (a) how many of the model's OpenRouter providers declare `reasoning` — anything short of all of them makes routing a lottery — and (b) whether a live request with `reasoning=false` actually stops the chain-of-thought. `--config <file>` checks every model a config can reach. Verified for `deepseek-v4-flash-0731`: 29/29 providers, honoured.
**Validation:** `parse_model_choices` fails loudly at startup on a non-list, an entry without `id`, duplicate labels, or a `reasoning` value that is not bool/mapping — a silently-dropped choice would leave students a chooser quietly missing an option.
**Key files:** `src/llm_client.py` (`_reasoning_kwargs`, applied in `stream_events` + `complete_json`), `src/course_loader.py` (`parse_model_choices`), `app.py` (`_reasoning_of`, `on_settings_update`, single-course choice list), `config_timeseries.yaml`, `scripts/check_reasoning_support.py`, `tests/model_choices.py`
**No-Goals:** Per-message thinking toggles, automatic model switching, exposing raw chain-of-thought in learning-goals mode (see IID-STREAM-RESILIENCE).
